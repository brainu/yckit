...
if($this->do=='comment'){
	$this->check_access('content_comment');
	$sql="SELECT * FROM ".DB_PREFIX."content_comment WHERE 1=1";
	if(!empty($_GET['article_id'])){
		$sql.=" AND article_id='".intval($_GET['article_id'])."'";
	}
	$sql.=" AND parent_id=0 ORDER BY comment_id DESC";
	$page_size=$this->config['content_admin_comment_list_size']?$this->config['content_admin_comment_list_size']:20;
	...
	if($count>0){
		foreach($res as $row){
			...
			$array[$row['comment_id']]['article_id']=$row['article_id'];
			$array[$row['comment_id']]['timestamp']=$row['comment_time'];
			$array2=array();
			$result2=$this->db->result("SELECT * FROM ".DB_PREFIX."content_comment WHERE parent_id='".$row['comment_id']."'");
			if($result2){
		...
		if(!empty($_GET['article_id'])){
			$parameters.="article_id=".intval($_GET['article_id'])."&";
		}
		$pager=pager(get_self(),$parameters,$page_current,$page_size,$count);
	}
	$this->template->in('comment',$array);
	$this->template->in('pager',$pager);
	$this->template->out('content.comment.list.php');
}
if($this->do=='comment_edit'){#编辑评论
	$comment_id=isset($_GET['comment_id'])?intval($_GET['comment_id']):'';
	$row=$this->db->row("SELECT * FROM ".DB_PREFIX."content_comment WHERE comment_id='$comment_id'");
	$comment=array();
	$comment['id']=$row['comment_id'];
	$comment['name']=$row['comment_name'];
	$comment['article_title']=$this->db->value(DB_PREFIX."content_article","article_title","article_id=".$row['article_id']);
	...
	$this->template->out('content.comment.info.php');
}
if($this->do=='comment_update'){#更新评论
	...
	$this->db->update(DB_PREFIX."content_comment",$array,"comment_id=$comment_id");
	if(!empty($_POST['comment_is_email'])){
		$to=trim($_POST['comment_email']);
		$subject="您在[".$this->config['title']."]的评论已经回复！";
		$article=$content->get_article_info($article_id);
		$content="评论文章：<a href='http://".$_SERVER['SERVER_NAME'].PATH.$content->category_parent_path($article['category_id'])."/".$article['html'].".html'>".$article['title']."</a><br />";
	...
	redirect('?action=content&do=comment');
}
if($this->do=='comment_delete'){#删除内容
	...
	foreach($comment_id as $value){
		if(!empty($value)){
			$row=$this->db->row("SELECT * FROM ".DB_PREFIX."content_comment WHERE comment_id='$value'");
			$this->db->query("UPDATE ".DB_PREFIX."content_article SET article_comment_count=article_comment_count-1 WHERE article_id=".$row['article_id']."");
			$this->db->delete(DB_PREFIX."content_comment","comment_id=".$value);
		}
	}
	if($this->config['content_mode']==1){
		$this->html_index();
...
if($this->do=='comment_audit'){
	...
	if(count($comment_id)>0){
		$this->db->query("UPDATE ".DB_PREFIX."content_comment SET comment_is_display=1 WHERE comment_id IN (".$comment_id.")");
	}
	if($this->config['content_mode']==1){
		$this->html_index();
	}
