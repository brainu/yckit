...
缩图宽度：<input tabindex="5" type="number" name="category_image_width" maxlength="3" style="width:60px" value="<!--{$category.image_width}-->" class="input"/>&nbsp;PX<p class="blank"></p>
		...
		<!--{if $mode=='update'}-->
		<p class="blank"></p>
		绑定菜单：<select name="menu_id" class="input">
		<option value="0">不绑定菜单</option>
		<!--{foreach from=$menu item=menu}--><option value="{$menu.id}" {if $menu.id==$category.menu_id}selected{/if}>{$menu.name}</option><!--{/foreach}-->
		</select>
		
		绑定菜单是指的在前台菜单选中效果，只对一级菜单有效
		<!--{/if}-->
		</fieldset>
